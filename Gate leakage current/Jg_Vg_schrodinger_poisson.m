...
psi_s_arr = 0.001:0.01:2;%logspace(log10(0.001),log10(2),200);%0.001:0.01:2;%
N = length(psi_s_arr);

J_g1 = zeros(length(t_oxide),N);
J_g2 = zeros(length(t_oxide),N);
Vg_arr = zeros(length(t_oxide),N);
...
psi_s = psi_s_arr(i)
%Vg1 = 1.3
[Vg, N_inv, F_ox_p] = schrodinger_poisson(m0,q,eps_sub,eps_ox,t_ox,N_sub,Eg,ni,psi_s,Ea);
...
%% ---------------------------
figure;
semilogy(Vg_arr(1,:),abs(J_g2(1,:))/1e4,'k:','linewidth',1.3);
hold on;
semilogy(Vg_arr(1,:),abs(J_g1(1,:))/1e4,'k','linewidth',1.1);
hold on;
semilogy(Vg_arr(2,:),abs(J_g2(2,:))/1e4,'k--','linewidth',2);
hold on;
...
title('Gate Current vs Gate voltage using Schrodinger-Poisson solver');
